...
async def cmd_list(event):
    ...
    if input_str == "text":
        string = (
            "Total {count} comandos encontrados en {plugincount} plugins de Skueletor\n\n"
        )
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            reply_text = f"**Se pueden ver todos los comandos de Skueletor [aquí]({url})**"
    ...
    if input_str:
        ...
        if HELPTYPE is True:
            help_string = f"Bot de ayuda... Diseñado por {DEFAULTUSER}\
                          \nPara revelar todos los nombres de los plugins\
                          \n__Utilice__ `.help` __plugin_name para comandos, en caso de que no aparezca.__\
                          \nUtilice `.info` plugin_name para comandos"
            ...
            string = "<b>Por favor, especifique para qué plugin desea obtener ayuda.\
...
async def info(event):
    input_str = event.pattern_match.group(1)
    if input_str == "text":
        string = "Total {count} comandos encontrados en {plugincount} plugins de Skueletor\n\n"
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            url = f"https://nekobin.com/{key}"
            reply_text = f"Para ver los comandos de Skueletor toca [aquí]({url})"
    ...
    if input_str:
        if input_str in SUDO_LIST:
            ...
            reply = await event.reply(input_str + " ¡El plugin no es válido!")
            await asyncio.sleep(3)
            await event.delete()
            await reply.delete()
    else:
...
async def info(event):
    ...
    if args:
        if args in CMD_HELP:
            ...
            event = await edit_or_reply(event, "Especifica un nombre de plugin válido.")
            await asyncio.sleep(3)
            await event.delete()
    else:
        string = "<b>Por favor, especifique para qué plugin desea obtener ayuda.\
